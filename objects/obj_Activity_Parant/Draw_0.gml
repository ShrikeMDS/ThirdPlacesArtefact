...
if (room == rm_Activity_Select_Caitlin)
{
	...
	else if (obj_Caitlin.var_money < var_cost)
	{
		draw_sprite(spr_Activity_Unavailable, 0, x, y)
	}
	else
	{
		draw_self();
...
if (room == rm_Activity_Select_Eugene)
{
	...
	else if (obj_Eugene.var_money < var_cost)
	{
		draw_sprite(spr_Activity_Unavailable, 0, x, y)
	}
	else
...
if (room == rm_Activity_Select_Maddie)
{
	...
	else if (obj_Maddie.var_money < var_cost)
	{
		draw_sprite(spr_Activity_Unavailable, 0, x, y)
	}
	else
...
if (room == rm_Activity_Select_Peter)
{
	...
	else if (obj_Peter.var_money < var_cost)
	{
		draw_sprite(spr_Activity_Unavailable, 0, x, y)
	}
	else
